    ...
    <table class="table table-bordered">
        ...
        @foreach ($paginate as $Mahasiswa)
            <tr>
                ...
                <td><img width="150px" src="{{ asset('storage/images/' . $Mahasiswa->image_profile) }}"> </td>
                <td>{{ $Mahasiswa->ttl }}</td>
                ...
                <td>
                    ...
                    <a type="submit" onclick="
                        event.preventDefault(); document.getElementById('form-delete').submit()
                    "
                    class="btn btn-danger text-white">Delete</a>

                    <form id="form-delete" action="{{ route('mahasiswa.destroy', $Mahasiswa) }}" method="post">
                        @csrf
                        @method('DELETE')
                    </form>
                </td>
